...
async function main() {
  ...
  const CastingVoteTx = await ballotContract.vote(VOTED_PROPOSAL);

  const CastingVoteTxReceipt = await CastingVoteTx.wait();
  console.log(`Voter ${wallet.address} voted for Proposal ${VOTED_PROPOSAL}`)
  console.log(
    `Transaction completed at block ${CastingVoteTxReceipt.blockNumber} with hash ${CastingVoteTxReceipt.blockHash}`
  );
}
