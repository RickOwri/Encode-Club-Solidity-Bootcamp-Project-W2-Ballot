...
async function main() {
  ...
  console.log(`The delegator for this ballot is ${wallet.address} and his delegated is ${COMPAGNON_DELEGATED}`)
  const DelegateRightToVoteTx = await ballotContract.delegate(COMPAGNON_DELEGATED);
  const DelegateRightToVoteTxReceipt = await DelegateRightToVoteTx.wait();
  console.log(
    `Transaction completed at block ${DelegateRightToVoteTxReceipt.blockNumber} with hash ${DelegateRightToVoteTxReceipt.blockHash}`
  );
}
